...
export interface MarrowDecisionBriefResult {
  period: { days: number; start: string; end: string };
  scope: { agent_id: string | null; session_id: string | null; role: 'deploy' | 'audit' | 'patch' | 'review' | 'general' };
  summary: string;
  risk: {
  ...
  handoff: { required: boolean; checkpoint_markers: string[]; stale_after_minutes: number };
  freshness: { check_required: boolean; surfaces: string[]; stale_context_warning: boolean };
  quality: { minimum_checks: string[]; outcome_required: boolean; score_floor: number };
  role_playbook: { role: 'deploy' | 'audit' | 'patch' | 'review' | 'general'; guidance: string[] };
  failure_alerts: Array<{ decision_type: string; message: string; severity: 'info' | 'warning' | 'critical' }>;
  proof_pack: { required: boolean; fields: string[] };
  source_of_truth: { required_surfaces: string[]; docs_required: boolean };
  fleet_reliability: { active_agents: number; outcome_coverage: number; measurement_risk: 'low' | 'medium' | 'high' };
  next_actions: string[];
}
